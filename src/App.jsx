...
const App = () => {
  ...
  return (
    <div className="flex min-h-screen bg-[#0b0f1a] text-white font-sans selection:bg-blue-500/30">
      ...
      {/* Sidebar - Identidade Visual Anúbis Tech */}
      <aside className="w-64 bg-[#161b2c] border-r border-slate-800 flex flex-col p-6 fixed h-full z-50 shadow-2xl">
        <h2 className="text-2xl font-black italic text-blue-500 mb-8 tracking-tighter uppercase leading-none font-black">
      ...
      {modalPagamento.aberto && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[200] flex items-center justify-center p-4 font-black">
          <div className="bg-[#161b2c] w-full max-w-sm rounded-[3rem] border border-slate-800 p-10 shadow-2xl animate-in zoom-in">
            <h3 className="text-xl font-black italic uppercase text-white tracking-tighter mb-8 text-center">Registrar Pagamento</h3>
            <div className="grid grid-cols-1 gap-3">
              {['pix', 'dinheiro', 'debito', 'credito'].map((metodo) => (
                <button 
                  key={metodo} 
                  onClick={() => finalizarComPagamento(metodo)} 
                  className="w-full p-4 rounded-2xl bg-slate-900 border border-slate-800 hover:bg-blue-600 transition-all font-black uppercase text-[10px] text-white tracking-widest"
                >
                  {metodo}
                </button>
              ))}
              <button onClick={() => setModalPagamento({ aberto: false, veiculo: null })} className="mt-4 text-slate-500 font-black uppercase text-[9px] hover:text-white w-full text-center">Cancelar</button>
            </div>
          </div>
        </div>
      )}
